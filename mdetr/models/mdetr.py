...
class MDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor=None, captions=None, forward_type="qga", **kwargs):
        ...
        """
        if forward_type=="mlm":
            out = self.transformer.text_encoder_lm(**kwargs)
            return out
        
        if forward_type=="contrastive":
            features, pos = self.backbone(samples)
            src, mask = features[-1].decompose()
            pooled_image_feats = self.input_proj(torch.mean(src.flatten(-2, -1), -1)[:, :, None, None])
            bs = pooled_image_feats.shape[0]
            text_embds1 = self.transformer.text_encoder(*captions[0]).last_hidden_state
            text_embds2 = self.transformer.text_encoder(*captions[1]).last_hidden_state
            mask1 = captions[0][1]
            pooled_text_feats1 = torch.stack([torch.mean(text_embds1[i,mask1[i]==1,:], dim=0) for i in range(bs)])
            mask2 = captions[1][1]
            pooled_text_feats2 = torch.stack([torch.mean(text_embds2[i,mask2[i]==1,:], dim=0) for i in range(bs)])
            text_out1 = self.contrastive_projection_text(pooled_text_feats1)
            text_out2 = self.contrastive_projection_text(pooled_text_feats2)
            image_out = self.contrastive_projection_image(pooled_image_feats.squeeze())
            return text_out1, text_out2, image_out
...
class ContrastiveCriterion(nn.Module):
    ...
    def forward(self, pooled_text, pooled_image, hard=False):
        ...
        if hard:
            bs = pooled_text.shape[0]
            logits_hard = torch.zeros((bs,2)).to(pooled_image.device)
            for i in range(bs):
                logits_hard[i, 0] = logits[i, i]
                logits_hard[i, 1] = torch.max(logits[i, list(range(i))+list(range(i+1,bs))])
            labels = torch.tensor([[1]*bs, [0]*bs], dtype=torch.float).T.to(pooled_image.device)
            loss_i = F.cross_entropy(logits_hard, labels)

            logits_hard_r = torch.zeros((bs,2)).to(pooled_image.device)
            for i in range(bs):
                logits_hard_r[i, 0] = logits.T[i, i]
                logits_hard_r[i, 1] = torch.max(logits.T[i, list(range(i))+list(range(i+1,bs))])
            labels_r = torch.tensor([[1]*bs, [0]*bs], dtype=torch.float).T.to(pooled_image.device)
            loss_t = F.cross_entropy(logits_hard_r, labels_r)
...
class QACriterionGQA(nn.Module):
    ...
    def forward(self, output, answers):
        ...
        #### cheating ####
        #output["pred_answer_type"] = F.one_hot(answers["answer_type"], num_classes=5).float()
        #### cheating ####
