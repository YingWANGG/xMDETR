...
from transformers import RobertaModel, RobertaTokenizerFast, RobertaForMaskedLM, RobertaForSequenceClassification
...
class Transformer(nn.Module):
    def __init__(
        ...
        text_tokenizer_type="roberta-base",
        ...
        print(text_tokenizer_type)
        print(text_encoder_type)
        self.tokenizer =  AutoTokenizer.from_pretrained(text_tokenizer_type) #RobertaTokenizerFast.from_pretrained(text_encoder_type)
        self.text_encoder = RobertaModel.from_pretrained(text_encoder_type)
        self.text_encoder.init_weights()
        # mlm
        self.text_encoder_lm = RobertaForMaskedLM.from_pretrained(text_encoder_type)
        self.text_encoder_lm.init_weights()
        self.text_encoder_lm.roberta = self.text_encoder
        self.text_encoder_lm.resize_token_embeddings(len(self.tokenizer))
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if isinstance(text[0], str):
                ...
                text_memory = encoded_text.last_hidden_state.transpose(0, 1)
                # Invert attention mask that we get from huggingface because its the opposite in pytorch transformer
                text_attention_mask = text[1].ne(1).bool()
...
def build_transformer(args):
    return Transformer(
        ...
        text_tokenizer_type=args.text_tokenizer_type,
