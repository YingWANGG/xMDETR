        ...
        if get_world_size() > 1:
            ...
            try:
                if not is_main_process():
                    ...
                    print(f"{get_rank()}: waiting for the {main_process_desc} to perform {desc}")
                    torch.distributed.barrier()
                yield
            finally:
                if is_main_process():
                    # the wait is over
                    print(f"{get_rank()}: {main_process_desc} completed {desc}, releasing all replicas")
                    torch.distributed.barrier()
        else:
            yield
