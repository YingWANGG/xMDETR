...
def build(image_set, args):
    ...
    #Change tokenizer
    tokenizer = AutoTokenizer.from_pretrained(args.text_tokenizer_type) #RobertaTokenizerFast.from_pretrained(args.text_encoder_type)
    ...
    if args.do_qa:
        ...
        if image_set == "train":
            ...
            for imset in ["train", "val"]:
                ...
                print(f"{image_set} path: {ann_file}")
                datasets.append(
            ...
            # Change path here
            ann_file = f"{args.gqa_ann_path}/{args.test_gqa_ann_path}/{image_set}_{args.gqa_split_type}_{args.lang}.json"
            print(f"{image_set} path: {ann_file}")
            return GQAQuestionAnswering(
                img_dir,
                ann_file,
        ...
        # Add few_shot
        elif image_set == "fewshot_train":
            # Change path here
            ann_file = f"{args.gqa_ann_path}/fewshot/{args.lang}/train_{args.n_shot}.json"
            print(f"{image_set} path: {ann_file}")
            return GQAQuestionAnswering(
                img_dir,
                ann_file,
                transforms=make_coco_transforms("train", cautious=True),
                return_masks=args.masks,
                return_tokens=True,
                tokenizer=tokenizer,
                ann_folder=Path(args.gqa_ann_path),
            )
        elif image_set == "fewshot_dev":
            # Change path here
            ann_file = f"{args.gqa_ann_path}/fewshot/{args.lang}/dev.json"
            print(f"{image_set} path: {ann_file}")
            return GQAQuestionAnswering(
                img_dir,
                ann_file,
        ...
        elif image_set == "fewshot_test":
            # Change path here
            ann_file = f"{args.gqa_ann_path}/fewshot/{args.lang}/test.json"
            print(f"{image_set} path: {ann_file}")
            return GQAQuestionAnswering(
                img_dir,
                ann_file,
